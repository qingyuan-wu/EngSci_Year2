...
% loop through to fill x values
for i = 1:N
    x(i) = start + h*(i-1);
end

for i = 1:N % loop through every element of x
...
% "Elapsed time is 0.081558 seconds."
...
% Elapsed time is 0.019517 seconds.
...
% y = A*exp(-sinx + x^3)
% y(0) = 1 => A = 1
% paricular sol'n: y = exp(-sinx + x^3)
...
%
% Details: After starting iode, click the "direction fields" button from 
...
% - The solutions are symmetric about the y-axis because dy/dx is a odd
% function: y'(-x) = -y'(x) due to the x term. Therefore, y is an even
% function.
