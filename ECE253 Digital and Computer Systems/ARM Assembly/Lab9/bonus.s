...
IRQ_HANDLER:
				...
				// IRQ FUNCTION - DETERMING WHAT KIND OF INTERRUPT OCCOURS 

PRIV_TIMER_CHECK:
				// R5 HOLDS INTERRUPT ID
...
PRIV_TIMER_ISR:	
				...
				MOV		R1, #1
				STR		R1, [R0, #12]  /* BASE ADRESS & TIMEOUT REGISTER IS 12 BITS AWAY FROM REGISTER - SO YOU OFFSET IT NY 12*/
				
				
				LDR R10, =513 /*TEST CASE */
				LDR R11, =258
...
CHK_KEY3:		CMP R1, #0b1000 // check key3
				 BNE END_KEY_ISR // If case happens we go to start_stop else we go to end_key_isr
...
START_STOP:		LDR		R0, =MPCORE_PRIV_TIMER		// timer base address
					...
					MVN R1, R1 // inverts r1 back to r1
					STR R1, [R0, #0x8] // STORE R1 timer control register
...
//switch mode - last 5 bit of cpsr  register tells us what mode we are in
			
			MOV R0, #0b10010 //change the mode from superviser to irq (interrupt)
			MSR CPSR, R0 //WE R IN IRQ NOW
			
			LDR SP, =0x100000
			MOV R0, #0b00010011 // Change mode to supervisor mode
			MSR CPSR , R0
			LDR SP, =0x200000 // we want stack pointer to be sufficiently far away from the memories we are using
			...
			// 8th bit of cpsr tells if were enabling the interrupt or not (0== enabeling 1== not enabelling)
...
CONFIG_PRIV_TIMER:
			...
			LDR R1, =50000000  // number of cycles
			STR R1, [R0] 
			
			MOV R1, #0b111 // the last one is start-stop, middle is autoreload, most dignificsnt one is interrupt-enable
			STR R1, [R0, #8] // 8 BECAUSE THATS WHERE THE CONTROL REGISTERS ARE LOCATED IN ADRESS MAP
			MOV 		PC, LR 					// return
...
CONFIG_KEYS:
			...
			MOV R1, #0b1000
			STR R1, [R0, #8] // THATS WHERE THE INTERRUPT ENABLES ARE LOCATED
